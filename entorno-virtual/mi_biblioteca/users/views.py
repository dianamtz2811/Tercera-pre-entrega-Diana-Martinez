...
def login_request(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user = authenticate(username=usuario, password=contrasenya)

            if user is not None:
                login (request, user)
                return render(request, "app_biblioteca/padre.html")

        msg_login = "Usuario o contraseña incorrectos, vuelva a intentarlo"
...
def editar_usuario(request):
    ...
    # y utilizando la instancia del usuario actual como base para la edición
        ...
        # Si se ha proporcionado una nueva imagen en el formulario
            if miFormulario.cleaned_data.get('imagen'): # Si el usuario ya tiene una imagen asociada (existe un objeto Imagen para este usuario)
